...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const sql = `INSERT INTO ${TABLE_NAMES.USERS} (username, email, password) VALUES (?, ?, ?)`;

    password = await bcrypt.hash(password, CONFIG.SALT_ROUNDS);

    await db.transaction(async (manager) => {
      await manager.query(sql, [username, "nah@gmail.com", password]);
    });
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { userName: username, password } = req.body as any;
    const sql = `SELECT users.*, new_user_roles.permissions FROM ${TABLE_NAMES.USERS} as users LEFT JOIN new_user_roles ON new_user_roles.id = users.roleId WHERE users.username = ? OR users.email = ?`;

    const [user] = await db.query(sql, [username, username]);

    if (!user) {
      throw new NotFound("User not found with email or username " + username);
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!page) {
      const userRoles = await NewUserRoles.find();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!page) {
      const users = await db.query(`SELECT * FROM ${TABLE_NAMES.USERS}`);

      const totalCount = await db.query(
        `SELECT COUNT(*) as totalCount FROM ${TABLE_NAMES.USERS}`
      );
      ...
      const users = await db.query(
        `SELECT users.*,new_user_roles.roleName FROM ${TABLE_NAMES.USERS} LEFT JOIN new_user_roles ON new_user_roles.id = users.roleId WHERE users.username LIKE '%${query}%' OR users.email LIKE '%${query}%'  LIMIT 10 OFFSET ${skip} `
      );

      const totalCount = await db.query(
        `SELECT COUNT(*) as totalCount FROM ${TABLE_NAMES.USERS} WHERE username LIKE '%${query}%' OR email LIKE '%${query}%'`
      );
